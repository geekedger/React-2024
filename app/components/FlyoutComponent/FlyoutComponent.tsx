...
const FlyoutComponent = () => {
  ...
  const selectedItems =
    useSelector((state: RootState) => state.selectedItems.items) || [];
  ...
  const handleDownload = () => {
    const csvContent =
      ...
      selectedItems
        .map((e) => {
          const sanitizedDescription = sanitizeDescription(e.description)
            .replace(/\n/g, " ")
            .replace(/\s+/g, " ")
            .trim(); // Применяем санитайзер и убираем новые строки
